# extraction_io/output_models.py
...
# 5) Now define ExtractionOutput as a true RootModel of the union
class ExtractionOutput(RootModel[Union[KeyValueOutput, BulletPointsOutput]]):
    root: Union[KeyValueOutput, BulletPointsOutput]


# 6) And define ExtractionOutputs as a RootModel of a list of ExtractionOutput
class ExtractionOutputs(RootModel[List[ExtractionOutput]]):
    root: List[ExtractionOutput]

    def dict_by_field(self) -> dict:
        """
        Return a simple { field_name: value_or_points } mapping for quick access.
